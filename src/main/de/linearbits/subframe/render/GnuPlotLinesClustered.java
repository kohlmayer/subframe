        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i == 0) {
                if (params.categorialX) {
                    command = "plot '" + filename + ".dat' using 2:xtic(1) with linespoints "+ params.lineStyle[i % params.lineStyle.length] +" "+color+"title col";
                } else {
                    command = "plot '" + filename + ".dat' using 1:2 with linespoints "+ params.lineStyle[i % params.lineStyle.length] +" "+color+"title col";
                }
            } else {
                if (params.categorialX) {
                    command = "     '' using " + (i + 2) + ":xtic(1) with linespoints "+ params.lineStyle[i % params.lineStyle.length] +" "+color+"title col";
                } else {
                    command = "     '' using 1:" + (i + 2) + " with linespoints "+ params.lineStyle[i % params.lineStyle.length] +" "+color+"title col";
                }
            }
